...
"""
用途：
    这是一个拿训练好分割模型的中间特征，比较 labeled 和 unlabeled 像素特征分布的 KDE 可视化脚本。

主要流程：
    1. 加载训练好的模型权重；
    2. 从 ACDC 训练集构造同时包含 labeled / unlabeled 的 batch；
    3. 前向推理得到分割预测 pred 和中间特征 BCP_feature；
    4. 将特征图展平成逐像素特征 [N, C]；
    5. 将标签 resize 到特征图大小，与预测结果逐像素对齐；
    6. 对指定类别中“预测正确”的像素分别从 labeled / unlabeled 部分采样；
    7. 对这些像素的特征做统计，并绘制 KDE 曲线，保存到本地。

输入要求：
    - 模型 forward 必须返回 (pred, feature)；
    - feature 的 shape 应为 [B, C, H, W]；
    - batch 中前 FLAGS.labeled_bs 个样本应来自 labeled 数据，其余来自 unlabeled 数据；
    - 需要提前准备好训练完成的模型权重文件。

输出结果：
    - 在 KDE/ACDC/... 目录下保存 KDE 可视化图像；
    - 图中绿色表示 labeled 样本特征分布，蓝色表示 unlabeled 样本特征分布。

典型用途：
    - 分析半监督分割中 labeled / unlabeled 特征是否对齐；
    - 观察某个类别的特征分布是否稳定；
    - 作为论文中的特征分布可视化实验脚本。

运行示例：
    python xxx.py \
        --root_path /data/byh_data/SSNet_data/ACDC \
        --labelnum 7 \
        --model unet \
        --num_classes 4 \
        --gpu 0 \
        --saved_path visible/previous_models/ACDC/

注意：
    - 当前脚本默认分析 class 2；
    - 标签 resize 时建议使用最近邻插值，避免类别标签被污染；
    - 该脚本是分析/可视化脚本，不负责训练模型。

KDE:
    - KDE 就是“平滑版直方图”，用来观察数据分布。
    - 放到你这个脚本里就是:用 KDE 比较 labeled 和 unlabeled 像素特征分布是否一致。
"""
